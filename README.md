...
### 💬 社交互动
- **点赞/收藏**：弹跳动画特效，实时计数更新
- **评论系统**：楼中楼嵌套回复，评论点赞动画，图片评论，实况图片评论，留币赠送
...
### 📡 实时通讯
- **私聊**：WebSocket 实时消息，文字/图片/实况图片/红包消息
- **群聊**：群号加入，群成员管理
- **红包消息**：微信风格红包发送与领取UI，渐变红色背景+金色按钮
- **实况图片**：聊天支持发送实况图片，左下角Live标识+点击播放
...
| 聊天页 | `chat_screen.dart` | 私聊/群聊，消息撤回，微信风格语音录制动画，红包发送/领取，实况图片，本地缓存(200条/会话) |
...
| 版本 | 构建号 | 说明 |
|------|--------|------|
| Beta 0.0.10 | 109 | 修复聊天红包/实况图片重复发送bug、修复评论区赠送留币逻辑bug（给谁送都显示"不能赠送自己"）、优化红包发送UI（渐变红色背景+金色按钮）、优化红包领取UI（圆形头像+金色金额+开红包按钮）、修复输入法遮挡聊天输入框、评论支持实况图片与留币赠送、APK混淆压缩打包(ABI split+ProGuard) |
